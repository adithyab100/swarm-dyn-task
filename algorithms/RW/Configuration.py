...
class Configuration:
	...
	torus: bool
		...
		edge effects
	"""
	def __init__(self, N, M, expected_demand_per_task, expected_time_until_task, time_per_demand, torus=False, pool = None):
		# Create all vertices
		self.vertices = {}
		for x in range(M):
			for y in range(N):
			...
			for y in range(self.N):
				if not self.vertices[(x,y)].state.is_home and not self.vertices[(x,y)].state.is_task:
					...
					if self.vertices[(x,y)].state.next_task_time == 0:
						...
						# print("new task", x,y, task_demand)
						self.vertices[(x,y)].state = VertexState(is_task = True, demand =task_demand, task_location = (x,y), task_timer = 0)
				...
				# remove task when completed and release all agents here
					...
					if self.vertices[(x,y)].state.residual_demand == 0:
						# print(x,y, self.vertices[(x,y)].state.demand)
